...
void downloadBlocks(string ip, string fileName, int startBlock, int lastBlock, int sizeOfLastBlock){
	...
	do{
		...
		if(listShownOnce==true){
			...
			if(command=="download"){
				...
				for(unsigned int i=0;i<localFileList[hashes[reqdIndex-1]].source.size();i++){
