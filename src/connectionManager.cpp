...
class ConnectionManager{
	...
	public:
		...
		{						
			char buffer[BLOCK_SIZE];
			bzero(buffer,BLOCK_SIZE+1);
			int read = recv(fd, buffer, BLOCK_SIZE, 0);
			if(DEBUG)
			{
			...
			//ofstream file2 (fileName+"2", ios::out | ios::binary);		         	
			if(DEBUG)
			{
			...
			{				
				...
				//fseek(reader,(startBlock*BLOCK_SIZE), SEEK_SET);
				...
				{				
					...
					//fread(buffer, 1,blockSize, reader);			
					//string blockNoString = to_string(i);
					//blockNoString.insert(0,16-blockNoString.size(),'0');
					if(DEBUG)
					{
						...
						/*for(int j=0;j<blockSize;j++)
							printf("%x ",buffer[j]);
						cout<<endl;*/
						//cout<<strlen(buffer)<<" "<<file.gcount();					
					}
					...
					if(DEBUG)
					{
		         	...
		         	/*if(file2.is_open())
		         	{
			         	...
			         	file2.seekp(i*BLOCK_SIZE,ios::beg);
			         	file2.write(buffer,blockSize);
